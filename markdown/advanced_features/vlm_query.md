...
```
    ...
    [2026-03-19 20:24:03] INFO utils.py:148: Note: detected 128 virtual cores but NumExpr set to maximum of 64, check "NUMEXPR_MAX_THREADS" environment variable.


    [2026-03-19 20:24:03] INFO utils.py:151: Note: NumExpr detected 128 cores but "NUMEXPR_MAX_THREADS" not set, so enforcing safe limit of 16.


    [2026-03-19 20:24:03] INFO utils.py:164: NumExpr defaulting to 16 threads.
...
```
    ...
    Model response:
    This image shows a price tag being hung out to dry on a rail attached to the back of a yellow taxi cab on a city street. The scene appears to be taking place in New York City, as indicated by the distinctive yellow cabs and the urban environment.
...
```
    ...
    The image shows a scene on a city street with two distinct taxi cabs. One cab is yellow and appears to be stopped or partially obstructed by another vehicle, while the second cab is a black SUV. The black SUV has a clothes rack attached to the back, which holds a shelf and some clothing items. This setup seems unusual and humorous, as clothing racks are typically used for luggage or pet carriers rather than carrying clothing for pedestrians. The scene suggests a playful or prank setup by the person controlling the clothing rack.
...
```
    ...
    The image shows a yellow taxi cab in a city street, with another yellow taxi cab slightly behind it. In the foreground, a person dressed in a yellow shirt and blue shorts is standing next to a捷克铰链立足装置（捷克铰链站），which is a vehicle-mounted toilet. The person appears to be using the toilet, suggesting that the vehicle is equipped with this particular type of toilet. The setting is a busy urban area with tall buildings and several street-level shops. There are also some flowers and decorative elements around, adding to the urban atmosphere.
