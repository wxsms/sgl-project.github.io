...
```
    ...
    [2026-03-17 19:49:47] INFO utils.py:148: Note: detected 128 virtual cores but NumExpr set to maximum of 64, check "NUMEXPR_MAX_THREADS" environment variable.


    [2026-03-17 19:49:47] INFO utils.py:151: Note: NumExpr detected 128 cores but "NUMEXPR_MAX_THREADS" not set, so enforcing safe limit of 16.


    [2026-03-17 19:49:47] INFO utils.py:164: NumExpr defaulting to 16 threads.
...
```
    ...
    [2026-03-17 19:49:59] Ignore import error when loading sglang.srt.models.glm_ocr: No module named 'transformers.models.glm_ocr'
    [2026-03-17 19:49:59] Ignore import error when loading sglang.srt.models.glm_ocr_nextn: No module named 'transformers.models.glm_ocr'
    [2026-03-17 19:49:59] Ignore import error when loading sglang.srt.models.glmasr: cannot import name 'GlmAsrConfig' from 'transformers' (/usr/local/lib/python3.10/dist-packages/transformers/__init__.py)
...
```
    ...
    Model response:
    This image shows a man setting up or taking down laundry on his street inching between two yellow taxis in a parking space.
...
```
    ...
    This image shows a yellow taxi driving in a city street. The taxi has two people hanging clothes on a clothesline attached to the back of the vehicle. The clothesline is holding various pieces of luggage and items, and there are at least two people standing on the back seat of the taxi. The city street has a few buildings and trees, and there are street lamps and traffic signals visible in the background. The scene appears to be part of a promotion or event, likely for a luggage company, as the clotheslines filled with items suggest that people are using taxis as makeshift laundry facilities.
...
```
    ...
    The image shows a street scene with several vehicles, including a yellow taxi cab. The background features a multi-story building with large windows displaying commercial signage. There are also several illuminated signs attached to the building. The street appears to be in an urban setting, likely in a city like New York, given the style of the taxi cab and the building design.
