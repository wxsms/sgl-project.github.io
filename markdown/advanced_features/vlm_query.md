...
```
    ...
    [2026-03-15 07:11:59] INFO utils.py:148: Note: detected 128 virtual cores but NumExpr set to maximum of 64, check "NUMEXPR_MAX_THREADS" environment variable.


    [2026-03-15 07:11:59] INFO utils.py:151: Note: NumExpr detected 128 cores but "NUMEXPR_MAX_THREADS" not set, so enforcing safe limit of 16.


    [2026-03-15 07:11:59] INFO utils.py:164: NumExpr defaulting to 16 threads.
...
```
    ...
    [2026-03-15 07:12:12] Ignore import error when loading sglang.srt.models.glm_ocr: No module named 'transformers.models.glm_ocr'
    [2026-03-15 07:12:12] Ignore import error when loading sglang.srt.models.glm_ocr_nextn: No module named 'transformers.models.glm_ocr'
    [2026-03-15 07:12:12] Ignore import error when loading sglang.srt.models.glmasr: cannot import name 'GlmAsrConfig' from 'transformers' (/usr/local/lib/python3.10/dist-packages/transformers/__init__.py)
...
```
    ...
    Model response:
    The image shows a person hanging clothes on the back of a yellow car in a busy city street. The car appears to be a taxi.
...
```
    ...
    The image shows a scene on a city street with a yellow taxi cab prominently in the foreground. The taxi has an item attached to its rear side, which appears to be a customized design or decoration. The background includes additional taxis, streetlights, and some buildings, indicating an urban setting. The scene suggests a busy city environment, possibly during a special event or celebration.
...
```
    ...
    This image shows two yellow cab taxis on the street. One taxi is relatively normal and stationary, while the other is featured prominently with a person standing near the back of its trunk. The person is wearing a yellow shirt and appears to be adjusting something in the trunk, possibly a tool or equipment. The background includes a building with multiple flags hanging outside, suggesting an urban setting. The scene seems staged, possibly for an event or demonstration.
