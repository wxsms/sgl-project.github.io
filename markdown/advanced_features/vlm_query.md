...
```
    ...
    [2026-03-16 01:10:24] INFO utils.py:148: Note: detected 128 virtual cores but NumExpr set to maximum of 64, check "NUMEXPR_MAX_THREADS" environment variable.


    [2026-03-16 01:10:24] INFO utils.py:151: Note: NumExpr detected 128 cores but "NUMEXPR_MAX_THREADS" not set, so enforcing safe limit of 16.


    [2026-03-16 01:10:24] INFO utils.py:164: NumExpr defaulting to 16 threads.
...
```
    ...
    In the image, there are two yellow taxis driving on a city street. One of the taxis has a person in the back seat using a broom or mop to clean the car. The person is wearing an yellow shirt. The street scene is likely set in an urban area, possibly during a day-to-night transition or a shift change when some taxi drivers might be cleaning or arranging their vehicles.
...
```
    ...
    From the image described, someone is standing behind a car, seemingly working with or pulling on a cloth or banner that appears to be attached to the car. The car is a yellow taxi cab, which is commonly used in New York City and other major U.S. cities. In the background, there is a building with multiple large banner or sign displays, which could indicate an event, promotion, or advertisement taking place.
