...
```
    ...
    [2026-03-10 22:09:18] INFO utils.py:148: Note: detected 128 virtual cores but NumExpr set to maximum of 64, check "NUMEXPR_MAX_THREADS" environment variable.


    [2026-03-10 22:09:18] INFO utils.py:151: Note: NumExpr detected 128 cores but "NUMEXPR_MAX_THREADS" not set, so enforcing safe limit of 16.


    [2026-03-10 22:09:18] INFO utils.py:164: NumExpr defaulting to 16 threads.
...
```
    ...
    Model response:
    This image shows a man hanging clothes on a line outside a building, vs. a taxi.
...
```
    ...
    The image depicts a scene on a city street with a yellow taxi driving down the road. The taxi has a large bag strapped to its back, which appears to be filled with clothes. A person, whose face is not visible, is wearing a yellow shirt and is pulling on the cage of the bag, possibly trying to secure or check the contents. The background shows buildings and some street signs, indicating an urban environment.
...
```
    ...
    The image shows a yellow taxi cab parked on the street. The taxi has flags attached to its back. A person, wearing a yellow shirt, is extending a blue flag or banner towards the front window of the taxi. The setting appears to be an urban area with buildings and trees visible in the background.
