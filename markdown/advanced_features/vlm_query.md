...
```
    ...
    [2026-03-14 20:07:18] Ignore import error when loading sglang.srt.models.glm_ocr: No module named 'transformers.models.glm_ocr'
    [2026-03-14 20:07:18] Ignore import error when loading sglang.srt.models.glm_ocr_nextn: No module named 'transformers.models.glm_ocr'
    [2026-03-14 20:07:18] Ignore import error when loading sglang.srt.models.glmasr: cannot import name 'GlmAsrConfig' from 'transformers' (/usr/local/lib/python3.10/dist-packages/transformers/__init__.py)
...
```
    ...
    Model response:
    The image shows a city street scene with two yellow taxis parked on the side of the road. Both taxis are part of the New York City taxi and limousine company, also known as the yellow cab bulletins. The scene also shows a person holding an iron and an ironing board, placing it on the tailgate of one of the taxis. This appears to be a humorous or whimsical scene, possibly used for a PTA (Parent-Teacher Association) event or a fun meeting.
...
```
    ...
    The image shows a scene with two yellow tour taxicabs parked next to a storefront with large glass windows. In front of the storefront, there is a person with a spray can performing spray painting art. The paint is being applied to bottoms of clothes hung on a rack that is attached to a three-legged stand. The person is wearing a yellow shirt. There are various signs and banners in the background, but the specific text on them is not clearly visible. The scene appears to be in an urban environment, possibly a cityscape.
...
```
    ...
    This image depicts a street scene in a bustling urban area. The most notable elements are two yellow taxis (yellow cabs) parked side by side on a city street. A person, who appears to be a man, is interacting with a hand-pulled laundry cart behind the rear right wheel of one of the taxis. The man has a casual stance, and he seems to be either loading or unloading clothes from the cart.
    
    The background includes other commercial buildings with large glass windows, some pedestrians walking by, and street lights and flags visible. The overall setting suggests a busy day in a city, likely during daytime given the brightness and the
