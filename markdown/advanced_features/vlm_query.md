...
```
    ...
    [2026-03-14 08:02:48] INFO utils.py:148: Note: detected 128 virtual cores but NumExpr set to maximum of 64, check "NUMEXPR_MAX_THREADS" environment variable.


    [2026-03-14 08:02:48] INFO utils.py:151: Note: NumExpr detected 128 cores but "NUMEXPR_MAX_THREADS" not set, so enforcing safe limit of 16.


    [2026-03-14 08:02:48] INFO utils.py:164: NumExpr defaulting to 16 threads.
...
```
    ...
    [2026-03-14 08:03:01] Ignore import error when loading sglang.srt.models.glm_ocr: No module named 'transformers.models.glm_ocr'
    [2026-03-14 08:03:01] Ignore import error when loading sglang.srt.models.glm_ocr_nextn: No module named 'transformers.models.glm_ocr'
    [2026-03-14 08:03:01] Ignore import error when loading sglang.srt.models.glmasr: cannot import name 'GlmAsrConfig' from 'transformers' (/usr/local/lib/python3.10/dist-packages/transformers/__init__.py)
...
```
    ...
    Model response:
    The image shows two yellow taxis parked on a city street. The focal point of the image is a man partially lifting aInserted object he is holding from a clothes rack attached to the back of a yellow taxi. The scene is set in an urban environment with buildings lining the street. The scene appears to have a humorous or unexpected element, given the unexpected activity with laundry in a city setting.
...
```
    ...
    The image shows a yellow taxi cab driving down a street. The driver appears to be hanging laundry out the window, possibly to dry it. The taxi is in posterior view, showing the back of the vehicle. The street is lined with buildings and there are flags visible, adding to the urban setting.
...
```
    ...
    This image shows a scene from a parade or public event taking place on a street. There are two yellow taxi cabs parked side-by-side. A person in a yellow shirt and jeans is demonstrating or interacting with a flag pole that is attached to the back of one of the taxis. The flag pole is decorated with various flags. The person appears to be arranging or a part of the display for the event. There are streetlights and other urban elements visible in the background.
