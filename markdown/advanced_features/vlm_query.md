...
```
    ...
    [2026-03-13 06:18:07] INFO utils.py:148: Note: detected 128 virtual cores but NumExpr set to maximum of 64, check "NUMEXPR_MAX_THREADS" environment variable.


    [2026-03-13 06:18:07] INFO utils.py:151: Note: NumExpr detected 128 cores but "NUMEXPR_MAX_THREADS" not set, so enforcing safe limit of 16.


    [2026-03-13 06:18:07] INFO utils.py:164: NumExpr defaulting to 16 threads.
...
```
    ...
    [2026-03-13 06:18:20] Ignore import error when loading sglang.srt.models.glm_ocr: No module named 'transformers.models.glm_ocr'
    [2026-03-13 06:18:20] Ignore import error when loading sglang.srt.models.glm_ocr_nextn: No module named 'transformers.models.glm_ocr'
    [2026-03-13 06:18:20] Ignore import error when loading sglang.srt.models.glmasr: cannot import name 'GlmAsrConfig' from 'transformers' (/usr/local/lib/python3.10/dist-packages/transformers/__init__.py)
...
```
    ...
    Model response:
    The image shows a man standing behind the back of two yellow and purple NYC taxis with a folding laundry cart. The man appears to be using a machine to fold clothes, and the scene is located in a high-visibility area, likely part of a New York City street filled with taxis during rush hour. The busy surroundings, high visibility, and the man's activity suggest that this setting could be a typical day and an unusual sight.
...
```
    ...
    This image shows a scene from a New York City parade, where a拉动的行军在街道上移动。拉力服由一个穿着黄色衬衫的人操作，车后有几面旗帜。场景与纽约市的特色出租车相似。
...
```
    ...
    The image you provided appears to be of two yellow taxi cabs parked on a city street. The taxis are typical of the New York City cab design, characterized by their distinctive bright yellow color, which is made up of multiple colors mixed together. In the background, you can see some buildings and what seems to be a storefront with some glass windows. There's also a striped awning on a building, possibly indicating a shop or café outside. The street looks busy, with the taxis parked along the curb, and the urban environment is typical of major downtown areas in cities like New York City. It seems to be filmed during the day
