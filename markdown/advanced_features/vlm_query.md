...
```
    ...
    [2026-03-22 04:33:03] INFO utils.py:148: Note: detected 192 virtual cores but NumExpr set to maximum of 64, check "NUMEXPR_MAX_THREADS" environment variable.


    [2026-03-22 04:33:03] INFO utils.py:151: Note: NumExpr detected 192 cores but "NUMEXPR_MAX_THREADS" not set, so enforcing safe limit of 16.


    [2026-03-22 04:33:03] INFO utils.py:164: NumExpr defaulting to 16 threads.
...
```
    ...
    Model response:
    The image shows a scene with two yellow taxis parked along a city street. A person is standing next to the taxi on the right, holding a drying rack or lap depending on how you interpret the way the items are arranged on the rack. There are clothes both on and off the rack, indicating it is being used for drying. It appears to be humorous, as drying clothes in the heat of the sun near a taxi is unusual. The background includes some commercial buildings with signs.
...
```
    ...
    The image shows a scene of two yellow taxis parked on a city street, possibly in New York City given the recognizable yellow cab color. The taxis are positioned next to a red barricade and some pedestrians can be seen in the background. The sidewalk includes a blue folding table covered with a piece of material, possibly tarps or fabric, draped over it, and some viewers are standing behind the barrier watching. The setting appears to be a busy urban environment with various signs and banners visible in the area.
...
```
    ...
    The image shows a street scene with three vehicles. The vehicles include a yellow taxi and two other cars. One of the cars appears to be an app-style vehicle, indicated by the presence of conveyed bags on a rack behind the windshield. There are also traffic cones on the street, likely indicating a controlled or restricted area. The background includes a building with flags hanging from its windows, suggesting an urban setting.
