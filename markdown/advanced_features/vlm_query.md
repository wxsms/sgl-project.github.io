...
```
    ...
    [2026-03-10 09:51:41] Ignore import error when loading sglang.srt.models.glm_ocr: No module named 'transformers.models.glm_ocr'
    [2026-03-10 09:51:41] Ignore import error when loading sglang.srt.models.glm_ocr_nextn: No module named 'transformers.models.glm_ocr'
    [2026-03-10 09:51:41] Ignore import error when loading sglang.srt.models.glmasr: cannot import name 'GlmAsrConfig' from 'transformers' (/usr/local/lib/python3.10/dist-packages/transformers/__init__.py)
...
```
    ...
    Model response:
    This image shows two taxi cabs in a city street, likely New York City, given the urban setting. The cabs are parked alongside the street. The scene is unusual and somewhat humorous, as there are clothes pegs attached to a clothesline running between the taxis. This indicates that the clothes have been hung up, suggesting some form of makeshift drying or perhaps even a temporary display.
...
```
    ...
    The image shows a scene on a street in an urban area. A yellow taxi cab is driving on the road. In the foreground, there is an oversized toy car balanced on a cart. A person is holding the cart, allowing the oversized toy car to stand upright. The person appears to be balancing the toy car on the cart. 
    
    The background includes other elements such as building facades, some signs, and some clothes hanging out of a shop window. The setting looks like a busy city street with commercial activity.
...
```
    ...
    The image shows a yellow taxi parked on a city street, with a person standing next to it. The person is holding a blue cloth, which appears to be a piece of fabric. The setting looks like a downtown area with buildings and trees in the background. The person seems to bebfocused on the cloth, possibly inspecting it.
