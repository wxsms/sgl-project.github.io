...
```
    ...
    [2026-03-10 11:29:25] INFO utils.py:148: Note: detected 192 virtual cores but NumExpr set to maximum of 64, check "NUMEXPR_MAX_THREADS" environment variable.


    [2026-03-10 11:29:25] INFO utils.py:151: Note: NumExpr detected 192 cores but "NUMEXPR_MAX_THREADS" not set, so enforcing safe limit of 16.


    [2026-03-10 11:29:25] INFO utils.py:164: NumExpr defaulting to 16 threads.
...
```
    ...
    [2026-03-10 11:29:39] Ignore import error when loading sglang.srt.models.glm_ocr: No module named 'transformers.models.glm_ocr'
    [2026-03-10 11:29:39] Ignore import error when loading sglang.srt.models.glm_ocr_nextn: No module named 'transformers.models.glm_ocr'
    [2026-03-10 11:29:39] Ignore import error when loading sglang.srt.models.glmasr: cannot import name 'GlmAsrConfig' from 'transformers' (/usr/local/lib/python3.10/dist-packages/transformers/__init__.py)
...
```
    ...
    Model response:
    This image shows a man dressed as a taxi cab with a beige suit as a taxi top tailgate, standing next to a regular yellow taxi in a city setting. The man is also carrying a red umbrella, which is an unusual and humorous addition to his outfit. The scene appears to be a creative act of humor or a performance of sorts, combining elements of public art with the iconic costumed imagery of taxis.
...
```
    ...
    The image shows a scene outdoors with a makeshift drying rack placed inside the door of a yellow taxi cab. The rack consists of a table made of metal or plastic and a piece of fabric draped over it to hang clothes. It appears that someone has set up ironing clothes on the fabric draped over the rack. The background includes streets, buildings, and other vehicles, indicating that this is likely taking place in an urban area. The yellow color of the taxi cab is distinctive in city settings.
...
```
    ...
    In the image, there are two taxi cabs on the street. Both cabs are yellow and are located on opposite sides of the street, facing the same direction. There are also several red and white flags hanging from the taxi cabs, which can be seen at various points along their trunks. These flags might be representing a particular event or cause. Additionally, there is a person in a yellow shirt hanging clothes on a hook attached to one of the cabs.
