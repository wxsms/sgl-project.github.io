...
```
    ...
    [2026-03-10 13:50:23] INFO utils.py:148: Note: detected 128 virtual cores but NumExpr set to maximum of 64, check "NUMEXPR_MAX_THREADS" environment variable.


    [2026-03-10 13:50:23] INFO utils.py:151: Note: NumExpr detected 128 cores but "NUMEXPR_MAX_THREADS" not set, so enforcing safe limit of 16.


    [2026-03-10 13:50:23] INFO utils.py:164: NumExpr defaulting to 16 threads.
...
```
    ...
    [2026-03-10 13:50:36] Ignore import error when loading sglang.srt.models.glm_ocr: No module named 'transformers.models.glm_ocr'
    [2026-03-10 13:50:36] Ignore import error when loading sglang.srt.models.glm_ocr_nextn: No module named 'transformers.models.glm_ocr'
    [2026-03-10 13:50:36] Ignore import error when loading sglang.srt.models.glmasr: cannot import name 'GlmAsrConfig' from 'transformers' (/usr/local/lib/python3.10/dist-packages/transformers/__init__.py)
...
```
    ...
    Model response:
    This is an image of a New York City street scene with several yellow taxis. The primary focus is on a man dressed in casual yellow attire, performing what appears to be a street performance or display involving multiple pairs of underwear. The man is standing on a small platform, surrounded by numerous pairs of underwear that cover most of his body. The background shows typical urban elements such as a storefront, buildings, and other traffic.
    
    The scene likely mimics a traditional street performance that can sometimes be found in New York, combining humor and underwear to entertain passersby.
...
```
    ...
    The image shows two yellow taxis on a city street. One taxi is driving on the left side of the street, and the other is stationary on the right side. The background includes tall buildings with large signs outside a store. The scene appears to be in an urban area with several banners and flags hanging from the poles. Additionally, a multi-colored object is being carried by a person on a metal bar (possibly a broom) attached to the back of one of the taxis.
...
```
    ...
    I'm not sure what exactly is shown in the image. It looks like there is a vehicle, possibly a yellow armchair or a similar object, but I don't have enough context to provide a detailed description. If you have any specific questions or details, please let me know!
