...
```
    ...
    [2026-03-20 16:22:00] INFO utils.py:148: Note: detected 192 virtual cores but NumExpr set to maximum of 64, check "NUMEXPR_MAX_THREADS" environment variable.


    [2026-03-20 16:22:00] INFO utils.py:151: Note: NumExpr detected 192 cores but "NUMEXPR_MAX_THREADS" not set, so enforcing safe limit of 16.


    [2026-03-20 16:22:00] INFO utils.py:164: NumExpr defaulting to 16 threads.
...
```
    ...
    Model response:
    The image shows a yellow New York City taxi cab being used for horseback riding. The individual is dressed in a racing number and appears to be promoting or celebrating horse racing. The billboard in the background mentions Rusty and Keenмест, which are well-known Thoroughbred racehorses. The horse is likely dressed in ceremonial attire, which is common at races.
...
```
    ...
    The image shows a person standing next to a yellow taxi cab, holding and using a pole to fold a blue blanket. It appears to be a demonstration or showcase of how to fold the blanket efficiently. The scene looks like it is set in an urban environment, possibly a city street, with several tall buildings and flags in the background.
...
```
    ...
    This image shows a scene from TV show "The Office." The scene depicts a man named Michael Scott, played by Steve Carell, using makeshift clothing racks to clean his clothes while standing in front of a yellow taxi cab. This humorous moment highlights Michael's eccentric or silly behavior. The setting appears to be the street in front of the Monster Co. headquarters.
