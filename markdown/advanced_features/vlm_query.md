...
```
    ...
    [2026-03-22 07:19:28] INFO utils.py:148: Note: detected 192 virtual cores but NumExpr set to maximum of 64, check "NUMEXPR_MAX_THREADS" environment variable.


    [2026-03-22 07:19:28] INFO utils.py:151: Note: NumExpr detected 192 cores but "NUMEXPR_MAX_THREADS" not set, so enforcing safe limit of 16.


    [2026-03-22 07:19:28] INFO utils.py:164: NumExpr defaulting to 16 threads.
...
```
    ...
    Model response:
    The image shows a street scene with two yellow taxis. One taxi has a clothes-drying rack mounted on its back, and a person is drying clothes on it. The scene suggests that the person might be resorting to drying their clothes on a public street to avoid additional costs. This is a common occurrence, especially in areas with limited access to dryers.
...
```
    ...
    It seems like there is an image of a man wearing a yellow shirt and blue shorts, standing on a street. There is a yellow car behind him, and he appears to be interacting with something hanging from a sign or flagpole. The background shows a city street with some buildings and flags.
...
```
    ...
    The image shows a street scene featuring two yellow taxis. One taxi is being cleansed by a person who is operating a pole-mounted steam cleaner. The cleaner is directed at the vehicle, and the steam is being used to clean the car. The scene is taking place in a busy urban area, as indicated by the buildings, street signs, and flags visible in the background. The overall setting suggests a demonstration or a public event, possibly related to municipal services or city maintenance.
