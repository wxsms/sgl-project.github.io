...
```
    ...
    [2026-03-12 05:41:55] INFO utils.py:148: Note: detected 128 virtual cores but NumExpr set to maximum of 64, check "NUMEXPR_MAX_THREADS" environment variable.


    [2026-03-12 05:41:55] INFO utils.py:151: Note: NumExpr detected 128 cores but "NUMEXPR_MAX_THREADS" not set, so enforcing safe limit of 16.


    [2026-03-12 05:41:55] INFO utils.py:164: NumExpr defaulting to 16 threads.
...
```
    ...
    [2026-03-12 05:42:08] Ignore import error when loading sglang.srt.models.glm_ocr: No module named 'transformers.models.glm_ocr'
    [2026-03-12 05:42:08] Ignore import error when loading sglang.srt.models.glm_ocr_nextn: No module named 'transformers.models.glm_ocr'
    [2026-03-12 05:42:08] Ignore import error when loading sglang.srt.models.glmasr: cannot import name 'GlmAsrConfig' from 'transformers' (/usr/local/lib/python3.10/dist-packages/transformers/__init__.py)
...
```
    ...
    Model response:
    The image shows a yellow taxi cab waiting in traffic. In the foreground, there is a person standing on the sidewalk, wearing glasses and a yellow shirt, with their back to the camera. They appear to be ironing clothes on a makeshift stand that looks like an umbrella. The person is holding an ironbox with both hands. Behind them, there is another yellow taxi taxi cab driving in the opposite direction. The scene appears to be in the middle of a busy city street with pedestrians and buildings in the background.
...
```
    ...
    I am not sure what first image, there are no images here.
...
```
    ...
    The image shows a yellow taxi driving on a city street. The taxi is using a hand crane to lift smaller objects, likely for garbage collection or other cleaning purposes. A person is handling the crane to perform this task. The scene appears to be in an urban area, and the buildings in the background suggest a commercial district.
