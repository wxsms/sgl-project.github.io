...
```
    ...
    [2026-03-10 17:37:56] INFO utils.py:148: Note: detected 128 virtual cores but NumExpr set to maximum of 64, check "NUMEXPR_MAX_THREADS" environment variable.


    [2026-03-10 17:37:56] INFO utils.py:151: Note: NumExpr detected 128 cores but "NUMEXPR_MAX_THREADS" not set, so enforcing safe limit of 16.


    [2026-03-10 17:37:56] INFO utils.py:164: NumExpr defaulting to 16 threads.
...
```
    ...
    [2026-03-10 17:38:08] Ignore import error when loading sglang.srt.models.glm_ocr: No module named 'transformers.models.glm_ocr'
    [2026-03-10 17:38:08] Ignore import error when loading sglang.srt.models.glm_ocr_nextn: No module named 'transformers.models.glm_ocr'
    [2026-03-10 17:38:08] Ignore import error when loading sglang.srt.models.glmasr: cannot import name 'GlmAsrConfig' from 'transformers' (/usr/local/lib/python3.10/dist-packages/transformers/__init__.py)
...
```
    ...
    Model response:
    The image shows a scene on a city street with two yellow taxis parked sideways, each suspended on hooks. The taxis appear to be part of a performance or street event, often referred to as "taxi art" or "taxi harrass." This是一种common sight in New York City, where drivers and passengers interact with the vehicles for comedic or artistic purposes. The person described as hanging clothes is likely part of this performance, creating a humorous or controversial scene.
...
```
    ...
    It looks like an outdoor art installation or a performance art piece where someone is hanging clothes on a public streetlight pole. The clothes appear to be hanging from the light and canopies (possibly striped sunshades or umbrellas). This kind of art is commonly performed in public spaces to highlight mundane or everyday objects and can make audiences look at familiar items with a fresh perspective.
...
```
    ...
    The image shows a yellow taxi driving down a city street. The taxi is equipped with a tricycle attachment on its back, which is used to ride and travel on roads, giving the driver greater maneuverability and stability. This type of taxi is particularly useful in areas with complex road configurations or tight spaces where standard taxis may have difficulties navigating.
