...
```
    ...
    [2026-03-14 15:04:46] INFO utils.py:148: Note: detected 128 virtual cores but NumExpr set to maximum of 64, check "NUMEXPR_MAX_THREADS" environment variable.


    [2026-03-14 15:04:46] INFO utils.py:151: Note: NumExpr detected 128 cores but "NUMEXPR_MAX_THREADS" not set, so enforcing safe limit of 16.


    [2026-03-14 15:04:46] INFO utils.py:164: NumExpr defaulting to 16 threads.
...
```
    ...
    [2026-03-14 15:04:58] Ignore import error when loading sglang.srt.models.glm_ocr: No module named 'transformers.models.glm_ocr'
    [2026-03-14 15:04:58] Ignore import error when loading sglang.srt.models.glm_ocr_nextn: No module named 'transformers.models.glm_ocr'
    [2026-03-14 15:04:58] Ignore import error when loading sglang.srt.models.glmasr: cannot import name 'GlmAsrConfig' from 'transformers' (/usr/local/lib/python3.10/dist-packages/transformers/__init__.py)
...
```
    ...
    Model response:
    A person is seen doing laundry on a clothesline draped over the back of a yellow taxi. The scene takes place on a city street, with other taxis and buildings visible in the background.
...
```
    ...
    The image shows a yellow taxi cab being gutted on a city street. The procedure involves removing the stationary and pedals, typically by unauthorized tinkerers for easy copying.
...
```
    ...
    The image shows a yellow taxi cab parked on a street. The person in the image is using a folding increment stand to extend the side doors of the taxi as a stand for a sign or some other object. The background includes typical urban elements such as commercial storefronts,街牌, 火車站停車場, and poles with American flags. The scene appears to be in an urban area, possibly in the United States.
