...
```
    ...
    [2026-03-12 23:22:15] INFO utils.py:148: Note: detected 128 virtual cores but NumExpr set to maximum of 64, check "NUMEXPR_MAX_THREADS" environment variable.


    [2026-03-12 23:22:15] INFO utils.py:151: Note: NumExpr detected 128 cores but "NUMEXPR_MAX_THREADS" not set, so enforcing safe limit of 16.


    [2026-03-12 23:22:15] INFO utils.py:164: NumExpr defaulting to 16 threads.
...
```
    ...
    [2026-03-12 23:22:27] Ignore import error when loading sglang.srt.models.glm_ocr: No module named 'transformers.models.glm_ocr'
    [2026-03-12 23:22:27] Ignore import error when loading sglang.srt.models.glm_ocr_nextn: No module named 'transformers.models.glm_ocr'
    [2026-03-12 23:22:27] Ignore import error when loading sglang.srt.models.glmasr: cannot import name 'GlmAsrConfig' from 'transformers' (/usr/local/lib/python3.10/dist-packages/transformers/__init__.py)
...
```
    ...
    Model response:
    The image shows a scene in a city with two yellow taxis, possibly New York City taxis, visible in the background. A person is hanging clothes on the back window of one of the taxis. The scene is quite unusual and humorous, as clothes are typically not hung on the back of a car. There are also several traffic signs visible in the background, including a green lane divider and red "Stop" signs. The setting is urban, with buildings and trees lining the street.
...
```
    ...
    The image shows two yellow taxis stopped on a city street. The taxis are attached to each other by a long elevated arm with a clothesline, which is holding a large blue and white merchandise banner. The arm is designed to lift the banner off the ground and hold it at an elevation. The scene appears to be a performance or demonstration, possibly for marketing or promotional purposes related to the content displayed on the banner. The background includes tall buildings and street lights, indicating an urban setting.
...
```
    ...
    The image shows two yellow taxis parked on the side of a street in an urban area. One of the taxis has a laundry cart attached to its back, and the cart is holding a piece of clothing or towel on it. The scene appears to be unusual or indicative of a specific local activity or movement being undertaken by the taxi driver. The background includes buildings and flags, suggesting a city location, possibly in North America given the style of the taxis and flags. The presence of the laundry cart could indicate a routine practice, a for-profit service, or an event.
