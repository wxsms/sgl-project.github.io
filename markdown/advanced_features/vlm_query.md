...
```
    ...
    [2026-03-12 09:14:23] Ignore import error when loading sglang.srt.models.glm_ocr: No module named 'transformers.models.glm_ocr'
    [2026-03-12 09:14:23] Ignore import error when loading sglang.srt.models.glm_ocr_nextn: No module named 'transformers.models.glm_ocr'
    [2026-03-12 09:14:23] Ignore import error when loading sglang.srt.models.glmasr: cannot import name 'GlmAsrConfig' from 'transformers' (/usr/local/lib/python3.10/dist-packages/transformers/__init__.py)
...
```
    ...
    Model response:
    The image shows two yellow taxis passing each other on a city street. One taxi appears to be picking up and placing a bag on its roof, while the other taxi remains stationary. The scene is set in a urban environment with buildings and store-fronts lining the street.
...
```
    ...
    This image shows two taxis on a city street, likely in New York City given the urban setting and the "6" on a sign that is commonly seen on taxicabs in that area. One taxi is yellow and closer to the viewer, while the other is blue. The yellow taxi has someone standing near the right rear tire, wearing a yellow shirt and appearing to be hanging some cloth or tarp over the open rear door. The person is wearing glasses, a yellow shirt, pants, and sneakers. There are alsolagen辆车l with some street signs visible.
...
```
    ...
    This image depicts a scene from a rain-soaked street in New York City, featuring two yellow taxis. In the foreground, a yellow car is displaying several rolls of paper towels on a bar pole, and a person is seen wearing a yellow shirt and a hat as they stand next to the display. In the background, another yellow car is partially visible. The scene captures the essence of a rainy day in New York City, with the characteristic yellow taxis and urban setting.
