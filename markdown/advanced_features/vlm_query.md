...
```
    ...
    [2026-03-14 04:52:34] INFO utils.py:148: Note: detected 128 virtual cores but NumExpr set to maximum of 64, check "NUMEXPR_MAX_THREADS" environment variable.


    [2026-03-14 04:52:34] INFO utils.py:151: Note: NumExpr detected 128 cores but "NUMEXPR_MAX_THREADS" not set, so enforcing safe limit of 16.


    [2026-03-14 04:52:34] INFO utils.py:164: NumExpr defaulting to 16 threads.
...
```
    ...
    [2026-03-14 04:52:46] Ignore import error when loading sglang.srt.models.glm_ocr: No module named 'transformers.models.glm_ocr'
    [2026-03-14 04:52:46] Ignore import error when loading sglang.srt.models.glm_ocr_nextn: No module named 'transformers.models.glm_ocr'
    [2026-03-14 04:52:46] Ignore import error when loading sglang.srt.models.glmasr: cannot import name 'GlmAsrConfig' from 'transformers' (/usr/local/lib/python3.10/dist-packages/transformers/__init__.py)
...
```
    ...
    Model response:
    This image depicts a scene from what appears to be a street performance or a quirky urban recreation commonly associated with performance art or vaudevillian entertainment. The main focus of the image involves a stand-it-up imitation machine set up between two yellow taxi cabs in what looks like a New York City street, given the iconic taxi design and the yellow color scheme, as well as the presence of traffic lights and an urban environment.
    
    The stand-it-up machine is configured as a vendor displays clothes in an Ironing Board setup. The performer is dressed in a bright yellow shirt, performing a magic trick to showcase a foldable ironing board that can double
...
```
    ...
    The image depicts a scene in an urban area with a pickup truck and a yellow taxi cab in the foreground. The truck appears to be moving along the street, with laundry draped over a platform or crane on its back. There is an apparent celebration or eccentric activity happening on the back of the truck. The background includes buildings with flags hanging from the windows and street lamps. The setting suggests a lively, possibly theatrical event or protest.
...
```
    ...
    The image shows a bright yellow and white taxi cab, which is a common mode of transportation in many big cities, such as New York. The cab appears to be parked on a street with other cars and buildings in the background. The scene looks like a typical urban setting with clear weather.
