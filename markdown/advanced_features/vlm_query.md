...
```
    ...
    [2026-03-22 08:33:59] INFO utils.py:148: Note: detected 192 virtual cores but NumExpr set to maximum of 64, check "NUMEXPR_MAX_THREADS" environment variable.


    [2026-03-22 08:33:59] INFO utils.py:151: Note: NumExpr detected 192 cores but "NUMEXPR_MAX_THREADS" not set, so enforcing safe limit of 16.


    [2026-03-22 08:33:59] INFO utils.py:164: NumExpr defaulting to 16 threads.
...
```
    ...
    Model response:
    The image shows a person wearing a yellow shirt who has a kind ofSprayon Laundry System, which they are using to clean clothes directly from the roof of their car. This isn't a typical practice, but in some innovative techniques or unusual scenarios, people do use various methods to wash clothes, like this approach that allows them to clean items right from their vehicle.
...
```
    ...
    This image shows a person standing in the rear of a yellow taxi cab, folding a piece of laundry. The scene appears to be set in a city with tall buildings, streetlights, and buildings with colorful awnings.
...
```
    ...
    The image shows a yellow taxi pulling over to the side of the road. There is a person dressed in a yellow shirt who is allesing a sign, which seems to be lifted using a tow bar attached to the back of the taxi. The scene appears to be in an urban environment, possibly in New York City, given the style of the taxi and the buildings in the background. The street is clean and well-maintained, with clear road markings and green trees lining the sidewalk.
