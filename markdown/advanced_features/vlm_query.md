...
```
    ...
    [2026-03-19 07:11:36] INFO utils.py:148: Note: detected 128 virtual cores but NumExpr set to maximum of 64, check "NUMEXPR_MAX_THREADS" environment variable.


    [2026-03-19 07:11:36] INFO utils.py:151: Note: NumExpr detected 128 cores but "NUMEXPR_MAX_THREADS" not set, so enforcing safe limit of 16.


    [2026-03-19 07:11:36] INFO utils.py:164: NumExpr defaulting to 16 threads.
...
```
    ...
    <frozen importlib._bootstrap_external>:1184: FutureWarning: The cuda.cudart module is deprecated and will be removed in a future release, please switch to use the cuda.bindings.runtime module instead.
    <frozen importlib._bootstrap_external>:1184: FutureWarning: The cuda.nvrtc module is deprecated and will be removed in a future release, please switch to use the cuda.bindings.nvrtc module instead.
    [2026-03-19 07:11:45] Transformers version 5.3.0 is used for model type qwen2_5_vl. If you experience issues related to RoPE parameters, they may be due to incompatibilities between Transformers >=5.0.0 and some models. You can try downgrading to transformers==4.57.1 as a workaround.


    [2026-03-19 07:11:50] Transformers version 5.3.0 is used for model type qwen2_5_vl. If you experience issues related to RoPE parameters, they may be due to incompatibilities between Transformers >=5.0.0 and some models. You can try downgrading to transformers==4.57.1 as a workaround.
...
```
    ...
    Model response:
    The image shows two taxis on a street. The two men are standing at the back of one of the taxis, working on something. The one on the left has rolled up a tarp or cloth, while the one on the right appears to be using some tools to secure or adjust the tarp. The setting looks like a busy urban area, possibly in a city where vehicles are used for service, given the common sight of workers in yellow vests in the same area.
...
```
    ...
    The image shows a man operating a home air drying system attached to the rear of a yellow taxi. The system is used to dry clothes after washing, and it appears to be attached to the back of the taxi, likely to take advantage of the relative cleanliness and airflow provided by the vehicle's exhaust system. The taxi is parked near a building with large windows, and there is another taxi visible in the background from the opposite direction.
...
```
    ...
    The image shows a yellow taxi cab driving down a street. The taxi is parked, and there is a batman costume draped over the back of the taxi, suggesting a costume party event or activity happening nearby. The taxi is stationed close to a building with large windows, and there are multiple flags hanging above the street. The street appears to be in an urban area, possibly in a large city.
