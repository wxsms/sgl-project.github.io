...
> [!IMPORTANT]
> **This site is deprecated and now serves only redirects.**
> The SGLang documentation has permanently moved to **<https://docs.sglang.io/>**.
> Every page in this repository is a small redirect stub (generated by
> [`make_redirects.py`](./make_redirects.py)) that forwards visitors to the same page on the
> new site; [`404.html`](./404.html) catches any other path. Please update your bookmarks.
>
> ⚠️ Documentation is authored in the [SGLang main repo](https://github.com/sgl-project/sglang)
> under `docs/` and was force-pushed here by its `release-docs.yml` workflow. That workflow must
> be disabled, otherwise the next docs build will overwrite these redirects. The notes below
> describe that (now superseded) pipeline.
